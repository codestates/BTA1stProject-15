...
route.post('/createWallet', function (req, res) {
  ...
  try {
    ...
    const createWallet = async () => {
      ...
      const HMnemonic = crypto
        .createHash('sha512')
        .update(mnemonic + salt)
        .digest('hex');

      const sql =
        'insert into test (HMnemonic, passwd, Address, salt) values ' +
        '("' +
        HMnemonic +
        '","' +
        hashPassword +
      ...
      await connection.query(sql, function (err, result) {
        let sync, responseData, responseDesc;
        sync = false;
        if (err) {
      ...
      var responseData = await {
        code: '0000',
        result: { mnemonic: mnemonic, address: address.address },
      };
      await res.send(responseData);
    };
      ...
      try {
        connection.query(sql, function (err, result) {
          ...
          if (
            ...
            responseData = {
              resultDesc: 'login email error : Email not Found',
              result: 'L_001',
            };
